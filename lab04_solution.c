...
double problem_3(double s1, double s2, double angle) {
    return 0.5 * s1 * s2 * sin(angle);
}
...
int problem_4(int n) {
    ...
    return problem_4(n - 1) + problem_4(n - 2);
}
...
unsigned int challenge(unsigned int n, unsigned int* memo) {
    ...
    memo[n] = challenge(n - 1, memo) + challenge(n - 2, memo);
    return memo[n];
}
